<?php
    ...
    $koneksi = conn($host, $username, $password, $database);
?>
<!--
...
<head>
    ...
    <title>Login - Kelola Surat</title>
    ...
    <style type="text/css">
        ...
        .bg::before {
            content: '';
            background-image: url('./asset/img/gedungft.jpeg');
                ...
                <div class="card-panel z-depth-2" id="login">
                    ...
                    <div class="col s12">
                        <div class="card-content">
                            <h5 class="center" id="title"><b>ARSIP SURAT</b></h5>
                            <?php echo '<img id="logo" src="upload/'.$data['logo'].'">';?>
                            <h4 class="center" id="ft">
                            <?php echo ''.$data['nama'].'';?>
                            </h4>
                            <div class="batas"></div>
                        </div>
                    ...
                    <?php
                        if(isset($_REQUEST['submit'])){
                            ...
                            if($_REQUEST['username'] == "" || $_REQUEST['password'] == ""){
                                echo '<div class="upss red-text"><i class="material-icons">error_outline</i> <strong>ERROR!</strong> Nama Pengguna dan Kata Sandi wajib diisi.
                                ...
                                $username = trim(htmlspecialchars(mysqli_real_escape_string($koneksi, $_REQUEST['username'])));
                                $password = trim(htmlspecialchars(mysqli_real_escape_string($koneksi, $_REQUEST['password'])));

                                $query = mysqli_query($koneksi, "SELECT id_user, username, nama, nip, admin FROM tbl_user WHERE username=BINARY'$username' AND password=MD5('$password')");
                                ...
                                if(mysqli_num_rows($query) > 0){
                                    ...
                                    //session error
                                    $_SESSION['errLog'] = '<center>Nama pengguna & Kata Sandi tidak ditemukan!</center>';
                                    header("Location: ./");
                                    die();
                                }
                            }
                    ...
                    <form class="col s12 m12 offset-4 offset-4" method="POST" action="" >
                        ...
                        <div class="input-field col s12">
                            <button type="submit" class="btn-large waves-effect waves-light blue darken1 col s12" name="submit">MASUK</button>
                        </div>
                    </form>
                    <!-- Form END -->
                    <?php
